...
function getFriendsJSON () {
  console.log('inizio json')
  function compareFn (a, b) {
    const aLength = parseInt(a.dataset.contentLen)
    const bLength = parseInt(b.dataset.contentLen)
    if (aLength > bLength) {
      return -1
  ...
  function findEdge (json) {
    console.log('cerco edge', _, json)
    return undefined
  }

  const injectedJSONsNodeList = document.querySelectorAll('script[type="application/json"][data-content-len]')
  const injectedJSONs = Array.from(injectedJSONsNodeList).sort(compareFn)
  const queryString = '"type":"FRIEND"'
  injectedJSONs.forEach(injectedJSON => {
    if (injectedJSON.innerText.search(queryString) > 0) {
      const edge = findEdge(JSON.parse(injectedJSON))
